...
* **Ownership metadata**: `OrganismRecord` carries both an `ownerOrganizationId` (collection owner) and an optional `managingOrganizationId` (facility currently caring for the stock). Defaults derive from the active organization but can be overridden for AZA/loan workflows.
