...
# Bootstrap .env from the community template if missing so dependent scripts work.
if [ ! -f .env ]; then
    if [ -f .env.community.example ]; then
        cp .env.community.example .env
        echo "Bootstrapped .env from .env.community.example (edit values as needed)."
    fi
fi
...
if lsof -Pi :9555 -sTCP:LISTEN -t >/dev/null 2>&1; then
    ...
    firebase emulators:start --only auth,firestore > firebase-emulator.log 2>&1 &
    EMULATOR_PID=$!
