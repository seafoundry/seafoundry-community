...
import 'package:seafoundry_community/errors/domain_errors.dart' as domain_errors;
...
import 'package:seafoundry_community/models/transfer_manifest_snapshots.dart';
...
import 'package:seafoundry_community/services/manual_transfer_registration_service.dart';
import 'package:seafoundry_community/services/outplant_geometry_builder.dart';
import 'package:seafoundry_community/services/provenance_crosswalk_service.dart';
import 'package:seafoundry_community/services/species_registry.dart';
import 'package:seafoundry_community/services/transfer_notification_result.dart';
import 'package:seafoundry_community/utils/js_error_utils.dart';
...
class TransferService implements ManualTransferRegistrationService {
  ...
  /// genet record based on the manifest payload. Validates that the manifest
  /// targets the current organization before mutating any data so the receiving
  /// dialog can surface actionable errors when the payload is stale.
  ...
  ///
  /// TODO(scaling): this fetches the 500 most-recent outbound transfer events
  /// (any status) and filters to pending/shipped in-memory. Firestore caps
  /// `whereIn` at one per query (already used for `eventTypeId`), so adding
  /// `where('status', whereIn: [pending, shipped])` is not allowed, and no
  /// events+status composite index exists. For orgs whose outbox exceeds the
  /// cap, split into two queries (one per status) and merge — requires a
