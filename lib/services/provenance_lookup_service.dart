...
class ProvenanceLookupService {
  ...
  /// Perform an exact alias lookup and return the first matching suggestion,
  /// or `null` if no match exists.
  ///
  /// [aliasType] lets the BLoC specify which field the value came from.
  Future<ProvenanceSuggestion?> exactLookup({
    ...
    try {
      final results = await _crosswalkService.lookupAlias(
        value,
        speciesCode: speciesCode,
      );
      ...
      // Fetch the full record to get speciesCode and accurate alias details.
      final record = await _crosswalkService.getProvenanceRecord(
        firstResult.provenanceId,
      );
      if (record == null) return null;

      final normalizedValue = _normalize(value);
      final matchingAlias = record.aliases
          .where((alias) => _normalize(alias.id) == normalizedValue)
          .firstOrNull;

      final otherAliases = record.aliases
          .where((a) => a.id != (matchingAlias?.id ?? value))
          .map((a) => a.id)
          .take(3)
          .toList();
      ...
      return ProvenanceSuggestion.fromRecordAlias(
        provenanceId: record.provenanceId,
        speciesCode: record.speciesCode,
        aliasId: matchingAlias?.id ?? value,
        aliasOrg: matchingAlias?.org ?? firstResult.org,
        aliasOrgType: matchingAlias?.orgType ?? firstResult.orgType,
        aliasTypeOverride: aliasType,
        otherAliases: otherAliases,
        masterClonalId: record.masterClonalId,
        resolvedClonalId: resolved.clonalId,
      ...
      _logger.error(
        'ProvenanceLookupService.exactLookup failed for '
        'value="$value", speciesCode="$speciesCode"',
        e,
        stackTrace,
      );
      return null;
