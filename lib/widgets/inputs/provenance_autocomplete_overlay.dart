...
import 'package:seafoundry_community/widgets/inputs/provenance_suggestion_tile.dart';

/// Positioned overlay dropdown for [ProvenanceAutocompleteField].
/// Wraps content in [CompositedTransformFollower] + [Material] elevation card.
class ProvenanceAutocompleteOverlay extends StatelessWidget {
  const ProvenanceAutocompleteOverlay({
    super.key,
    required this.link,
    required this.width,
  ...
  Widget _buildBody(BuildContext context) {
    ...
    if (suggestions.isNotEmpty) {
      return ListView.builder(
        ...
        itemBuilder: (_, i) => ProvenanceSuggestionTile(
          suggestion: suggestions[i],
          isHighlighted: i == highlightedIndex,
          onTap: onSuggestionSelected,
          displayValueBuilder: displayValueBuilder,
          showOtherAliases: showOtherAliases,
        ),
      );
    }
    if (valueLength >= 2) {
