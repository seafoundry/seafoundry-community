...
import 'package:seafoundry_community/models/factories/event_factory.dart';
...
import 'package:seafoundry_community/services/stream_cache.dart';
import 'package:seafoundry_community/utils/date_time_converter.dart';
import 'package:seafoundry_community/repositories/utils/firestore_document_helpers.dart';
...
class EventRepository extends _EventRepositoryBase {
  ...
  /// Creates the MoveInEvent for a record that was moved.
  ///
  /// This method is ONLY ever called inside an outer [Transaction]. The event
  /// [slug] MUST be pre-minted OUTSIDE that transaction (see
  /// `InventoryRecordRepository.nextSlugForBase` and the GraphRepository move
  /// planner) and passed in — minting a slug here would open a nested
  /// transaction, which the Firestore Web SDK does not support.
  Future<MoveInEvent> createMoveInEvent(
    ...
    final String moveInEventId = eventId ?? generateId(firestore: db);
    final String moveInSlug = slug;
  ...
  /// Creates the MoveOutEvent for a record that was moved.
  ///
  /// This method is ONLY ever called inside an outer [Transaction]. The event
  /// [slug] MUST be pre-minted OUTSIDE that transaction and passed in — minting
  /// a slug here would open a nested transaction, which the Firestore Web SDK
  /// does not support.
  Future<MoveOutEvent> createMoveOutEvent(
    ...
    final String moveOutEventId = eventId ?? generateId(firestore: db);
    final String moveOutSlug = slug;
  ...
  Future<PhysicalFormChangeEvent> createPhysicalFormChangeEvent({
    ...
    var event = PhysicalFormChangeEvent(
      ...
      recordId: recordId,
      recordModelType: ModelType.organismRecord,
      organismRecordSnapshot: PhysicalFormSnapshot.fromOrganismRecord(snapshot),
      oldFormId: oldFormId,
      newFormId: newFormId,
      metadata: metadata,
    );
  ...
  Future<QuantityChangeEvent> createQuantityChangeEvent({
    ...
    var event = QuantityChangeEvent(
      ...
      recordId: recordId,
      recordModelType: ModelType.organismRecord,
      organismRecordSnapshot: QuantityChangeSnapshot.fromOrganismRecord(snapshot),
