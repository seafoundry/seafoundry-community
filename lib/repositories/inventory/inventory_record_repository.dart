...
import 'package:seafoundry_community/repositories/auth_error_suppression_mixin.dart';
import 'package:seafoundry_community/services/firestore_recovery_service.dart';
...
import 'package:seafoundry_community/services/url_path_resolver.dart';
import 'package:seafoundry_community/utils/stream_factory.dart';
import 'package:seafoundry_community/widgets/repositories/registry/disposable.dart';
...
abstract class InventoryRecordRepository<T extends InventoryRecord>
  ...
  /// Opens its own runTransaction; safe ONLY when awaited OUTSIDE an outer
  /// runTransaction. Pre-mint before entering a transaction (see premintSlugs).
  Future<String> nextSlugForModelType(ModelType modelType) async {
    return nextSlugForBase(modelType.name);
  }

  /// Pre-mints [count] slugs for [type], sequentially, each in its own
  /// transaction. Call this OUTSIDE an outer transaction, then pass the results
  /// into the transaction body. Sequential (not Future.wait) to avoid
  /// self-contention on the same counter doc.
  Future<List<String>> premintSlugs(ModelType type, int count) async {
    final slugs = <String>[];
    for (var i = 0; i < count; i++) {
      slugs.add(await nextSlugForModelType(type));
    }
    return slugs;
  }
  ...
  Future<void> updateRecord(T record, {WriteBatch? batch}) async {
    ...
    // Preserve immutable createdById on updates.
  ...
  /// Moves [record] under [toParent] within an outer [transaction].
  ///
  /// Slug minting has been hoisted OUT of the transaction: the move event slugs
  /// ([moveOutSlug]/[moveInSlug]) and, for partial splits, the new organism slug
  /// ([partialNewOrganismSlug]) MUST be pre-minted by the caller (GraphRepository
  /// move planner) and passed in. This method performs only writes — it never
  /// opens a nested transaction. See docs at the top of this file.
  Future<T> moveRecord({
    required T record,
    ...
    if (isPartialMove && record is OrganismRecord) {
      assert(
        partialNewOrganismSlug != null && partialNewOrganismId != null,
        'Partial move requires a pre-minted organism slug and id',
      );
    ...
    final movedRecord = computeMovedRecordForMove(
      record: record,
      toParent: toParent,
      now: now,
    );
  ...
  /// Pure computation of the moved (full-move) record — no Firestore writes.
  ///
  /// Shared by [moveRecord] (the transactional write path) and by the
  /// GraphRepository move planner, which needs the moved record OUTSIDE the
  /// transaction to chain children onto their new parent. [now] is injected so
  /// the plan pass and the commit pass agree (paths are `now`-independent).
  T computeMovedRecordForMove({
    required T record,
    required GraphNodeRecord toParent,
    required String now,
  }) {
  ...
  /// Whether a move of [record] under [partialSelection] will split an organism
  /// (creating a new destination record and reducing the source). Public mirror
  /// of [_shouldSplitOrganism] so the out-of-transaction move planner can decide
  /// whether to pre-mint the extra organism slug.
  bool willSplitOnMove(T record, PartialMoveSelection? partialSelection) =>
      _shouldSplitOrganism(record, partialSelection);
  ...
  /// Pure computation of the new destination organism created by a partial
  /// (split) move — no Firestore writes. Shared by [_handlePartialMove] (the
  /// transactional write path) and the GraphRepository move planner, which needs
  /// the new organism OUTSIDE the transaction to chain children onto it. The
  /// [newSlug]/[newOrganismId] MUST be pre-minted/generated by the caller.
  OrganismRecord computePartialMovedOrganism({
    required OrganismRecord record,
    required GraphNodeRecord toParent,
    required int requestedQty,
    required String newOrganismId,
  ...
  ///
  /// Slugs/ids are pre-minted by the caller (see [moveRecord]); this method
  /// performs only writes and never opens a nested transaction.
  Future<OrganismRecord> _handlePartialMove({
    ...
    final newOrganism = computePartialMovedOrganism(
      record: record,
      toParent: toParent,
      requestedQty: requestedQty,
      newOrganismId: newOrganismId,
      newSlug: newSlug,
      now: now,
    );
