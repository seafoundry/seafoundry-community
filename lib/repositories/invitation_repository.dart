...
class InvitationRepository {
  ...
  Future<String?> acceptInvitationAtomic({
    ...
    return await db.runTransaction<String?>((transaction) async {
      ...
      // Consume the deterministic invitation-index doc that authorized this
      // member-create under the security rules (keyed {orgId}_{emailLower}).
      // No-op if none exists. Whoever creates an invitation must also write the
      // matching invitation_index doc; see firestore.rules.
      final indexRef = db
          .collection('invitation_index')
          .doc('${invitation.organizationId}_$normalizedEmail');
