...
// Canonical project-id resolution. Mirrors scripts/lib/require-emulator-or-demo.js
// (FIREBASE_PROJECT_ID first, then GCLOUD_PROJECT, then GOOGLE_CLOUD_PROJECT).
...
// Fail-safe guard: only run against emulators or demo-* project IDs.
// Mirrors scripts/lib/require-emulator-or-demo.js. Returns the resolved project
// id so the caller passes the EXACT validated id to initializeApp().
function requireEmulatorOrDemo(
  scriptName: string,
  explicitProjectId?: string,
): { isEmulator: boolean; projectId: string } {
  const isEmulator = !!(
  ...
  if (!isEmulator && !isDemoProject) {
    ...
    console.error(`     npx ts-node scripts/firestore/${scriptName}`);
    console.error('');
...
function initializeFirestore(projectId: string): Firestore {
...
async function main() {
  const { projectId } = requireEmulatorOrDemo('seed_taxonomy_data.ts');
  ensureSeedCoverage();
