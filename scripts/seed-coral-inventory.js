...
// Community fork ships no crosswalk data; this gracefully no-ops. When present,
// the file is an optional local export placed under crosswalk_data/ at the repo
// root (never committed to this repo).
function loadCrosswalkData() {
  const bySpecies = {};
  try {
    const crosswalkPath = path.resolve(__dirname, '..', 'crosswalk_data', 'crosswalk.json');
    if (!fs.existsSync(crosswalkPath)) {
      console.log('INFO: No crosswalk data found at crosswalk_data/crosswalk.json');
      return bySpecies;
    }
...
// with real provenance IDs from an external registry which use the production species codes.
